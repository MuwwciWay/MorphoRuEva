...
def process_token(token, label, prev_char, is_start_of_sentence):
    """Обработка токенов в зависимости от их метки."""
    new_token = token
    substitution = None
    ...
    # Проверка на начало нового предложения
...
def punctuate_and_get_symbols(sentence):
    ...
    for index, item in enumerate(preds):
        is_start_of_sentence = (index == 0)  # Определяем, является ли токен началом предложения
...
def compare_labels(original_text, modified_text):
    ...
    for i in range(len(original_words)):
        ...
        if '(' in word and ')' in word:  
            ...
            # Ищем правую границу в модифицированном тексте
...
# Сравнение значений и создание нового столбца 'result'
df['result'] = df.apply(lambda row: 1 if row['student_answer'] == row['filtered_punctuation_indices'] else 0, axis=1)

# Вычисление процента правильно определённых знаков
df['procent'] = df.apply(lambda row: (len(row['filtered_punctuation_indices']) / len(row['student_answer'])) * 100 if len(row['student_answer']) > 0 else 0, axis=1)
